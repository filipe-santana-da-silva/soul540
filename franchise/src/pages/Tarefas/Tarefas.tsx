...
export default function Tarefas() {
  ...
  const tasksByStatus = useMemo(() => {
    const map: Record<TaskStatus, typeof tasks> = { backlog: [], todo: [], in_progress: [], review: [], done: [] };
    filtered.forEach((t) => {
      const s = (t.status as TaskStatus) || 'backlog';
                  ...
                  tasksByStatus[col.id].map((task) => (
                    ...
                    >
                      <div className={styles.taskTop}>
                        <Badge variant={priorityColors[task.priority as TaskPriority] || 'gray'}>{priorityLabels[task.priority as TaskPriority] || task.priority}</Badge>
            ...
            filtered.map((task) => (
              <div key={task.id} className={styles.listItem}>
                ...
                <div className={styles.listItemBadges}>
                  <Badge variant={priorityColors[task.priority as TaskPriority] || 'gray'}>{priorityLabels[task.priority as TaskPriority] || task.priority}</Badge>
                  <Badge variant={statusColors[task.status as TaskStatus] || 'gray'}>{statusLabels[task.status as TaskStatus] || task.status}</Badge>
            ...
            <div className={styles.modalBody}>
              <div className={styles.infoSection}>
                ...
                <p className={styles.infoText}>
                  A página de <strong>Tarefas</strong> permite gerenciar todas as atividades em um quadro Kanban visual ou em lista. As tarefas podem ser vinculadas a eventos específicos e organizadas por prioridade.
                </p>
              </div>
              <div className={styles.infoSection}>
              ...
              <div className={styles.infoSection}>
                ...
                <ol className={styles.infoList}>
                  ...
                  <li>Na <strong>Lista</strong>, visualize o status de cada tarefa.</li>
                </ol>
              </div>
            </div>
            <div className={styles.modalFooter}>
